...
class BusinessNewsPage extends StatefulWidget {
  const BusinessNewsPage({super.key});

  @override
  State<BusinessNewsPage> createState() => _BusinessNewsPageState();
}

class _BusinessNewsPageState extends State<BusinessNewsPage> {
  Future<void> _refreshNews() async {
    await Future.delayed(const Duration(seconds: 1));
    BlocProvider.of<HomeBloc>(context).add(const GetBusinessNews(link: Constants.businessNewsLink));
  }
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<HomeBloc, HomeState>(
      builder: (context, state) {
        if (state.businessNewsStatus == Status.success) {
          var news = state.businessNews;
        ...
        } else if (state.businessNewsStatus == Status.error) {
          return SizedBox(
            height: 500,
            child: MyErrorWidget(
              onRetry: () {
